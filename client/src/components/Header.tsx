                  ...
                  <div className="flex justify-between py-4 mb-4 border-b-2">
                        ...
                        { isLoggedIn ? (
                          <div className="flex items-center gap-4">
                                ...
                                { !isAdmin && (
                                  <Link to="/cart" className={`p-1 mr-6 relative rounded-full text-gray-700 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${activeLink === "cart" && "text-indigo-500"}`}>
                                ...
                                <Link to="/auth/profile" className={`flex grow ml-4 text-sm font-medium text-gray-700 relative ${activeLink === "profile" && "text-indigo-500"}`}>
                                      <User />
                                      <span className="absolute -top-4 right-0 text-nowrap">{currentUser && currentUser.name?.split(' ')[0]}</span>
                                </Link>
                          </div>
                        ) :   <Link
                                to="/auth/login"
